...
const AcceptedDonations = () => {
  ...
  const fetchDonations = async () => {
    ...
    try {
      ...
      const mappedDonations = res.data.map(donation => ({
        ...
        otpRequested: false,
        otpInput: '',
        pickedUp: donation.status === 'distributed' ? true : false
      }));
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'picked-up': return < Tick size={16} className="status-icon picked-up" />;
        ...
        <div className="filter-section">
          <div className="filter-dropdown">
            ...
            >
              ...
              <option value="expired">Picked-Up</option>
            </select>
          </div>
        </div>
      </div>
      ...
      try {
        ...
        if (res.data.success) {
          setDonations(prev =>
            prev.map(d =>
              d.id === donation.id
                ? { ...d, otpRequested: true, pickupId: res.data.pickupId, otpInput: '' }
                : d
            )
          );
      ...
      onClick={async () => {
        try {
          ...
          if (res.data.success) {
            setDonations(prev =>
              prev.map(d =>
                d.id === donation.id ? { ...d, pickedUp: true, status: 'picked-up' } : d
              )
            );
            alert(res.data.message);
          } else {
